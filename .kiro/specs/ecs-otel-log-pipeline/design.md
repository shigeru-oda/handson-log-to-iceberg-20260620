...
2. **ルーティングレイヤ (FireLens / Fluent Bit)**: stdout を受信し、`log` 文字列を JSON 展開した上で OTel `severityText` (`ERROR`/`FATAL`) に基づき配信先を分岐 (Req 3)。Iceberg 系統は Lua で小文字フラットスキーマへ整形する。
...
> **重要 (FireLens のレコード構造と既知の罠)**
> `awsfirelens` ログドライバは、アプリ stdout の 1 行を record の `log` キーに **文字列のまま**
> 格納する (例: `{"log":"{\"severityNumber\":17,...}","container_name":"app",...}`)。
> そのため `severityNumber` / `severityText` はトップレベルに存在せず、まず `parser` フィルタで
> `log` を JSON 展開する必要がある。さらに **`rewrite_tag` の正規表現は文字列フィールドにしか
> マッチせず、整数の `$severityNumber` には一致しない**。したがって判定は文字列の
> `$severityText` (`^(ERROR|FATAL)`) で行う。これを怠ると ERROR/FATAL が CloudWatch にも
> Iceberg にも一切配信されない (無音故障)。

設定アプローチ (実装は `fluent-bit/custom.conf` + `parsers.conf` + `iceberg_transform.lua` を
カスタムイメージにベイク):

```ini
# [INPUT] は FireLens が forward で自動注入する (awsfirelens 経由のアプリ stdout)

# [SERVICE] でカスタム parser を追加ロード (FireLens 生成 config へ @INCLUDE 時にマージ)
[SERVICE]
    Parsers_File /fluent-bit/etc/parsers.conf

# 0) "log" 文字列を JSON 展開してトップレベルへ (Reserve_Data で ECS メタデータ保持)
[FILTER]
    Name         parser
    Match        app-firelens*
    Key_Name     log
...
# 1) severityText が ERROR/FATAL の Error_Log に error.* タグを付与 (元タグは保持 = true)
#    ※ 整数 $severityNumber は rewrite_tag にマッチしないため文字列 $severityText で判定
[FILTER]
    Name          rewrite_tag
    Match         app-firelens*
...
# 2) Iceberg 向けに error.* を iceberg.* へ複製 (CloudWatch は素の OTel を残す)
[FILTER]
    Name          rewrite_tag
    Match         error.*
...
# 3) iceberg.* のみ Lua で小文字フラットスキーマへ整形 (Iceberg_Schema_Mapping)
[FILTER]
    Name    lua
    Match   iceberg.*
...
# 4) 全ログ -> Firehose (S3 full-logs)  ※派生コピーを除外し重複回避 (Req 4.2, 4.3)
[OUTPUT]
...
# 5) Error_Log -> CloudWatch Logs (素の OTel エラーレコード) (Req 4.1)
[OUTPUT]
...
# 6) Error_Log -> Firehose (S3 Tables Iceberg / 整形済み) (Req 5.2)
[OUTPUT]
    Name            kinesis_firehose
    Match           iceberg.*
    region          ap-northeast-1
...
# 7) Error_Log -> Firehose (Glue Iceberg / 整形済み) (Req 6.3)
[OUTPUT]
    Name            kinesis_firehose
    Match           iceberg.*
    region          ap-northeast-1
...
> 補足: タグの派生は 2 段階。`app-firelens*` → (ERROR/FATAL) → `error.*` → (整形用コピー)
> → `iceberg.*`。各 `rewrite_tag` は第4引数 `true` で元レコードを保持するため、
> full-logs (`app-firelens*` 全件)、CloudWatch (`error.*` 素の OTel)、Iceberg
> (`iceberg.*` 整形済み) が重複なく両立する。severity 判定は文字列 `$severityText`
> (`^(ERROR|FATAL)`) で行う (整数 `$severityNumber` は rewrite_tag にマッチしないため不可)。
> Go アプリ側の正典ロジック (`routing.go`: `severityNumber >= 17`) とは判定基準が等価。
...
> **変換の実施箇所 (重要)**
> Firehose の Iceberg 配信はレコードのキー名を宛先テーブルのカラム名に一致させる必要が
> あるが、アプリが出力する OTel JSON は camelCase かつネスト構造 (`severityNumber` /
> `resource` 等) であり、そのままでは小文字フラットスキーマにマッピングできない。
> このマッピングは **FireLens の Lua フィルタ (`fluent-bit/iceberg_transform.lua`)** で行う
> (Firehose の data transformation Lambda は使用しない)。Lua は parser 展開済みレコードから
> 上表の 7 カラムだけを持つレコードを生成し、`resource`/`attributes` を JSON 文字列化、
> `event_time` をタイムゾーンなし・マイクロ秒の ISO8601 に整形、`ingest_date` を
> `YYYY-MM-DD` で導出する。`iceberg.*` タグのレコードにのみ適用し、full-logs / CloudWatch
> 系統 (素の OTel) には影響しない。
...
- **S3 Tables Iceberg** (`S3_Tables_Iceberg`): namespace + table を S3 Tables バケット内に作成。カラム名は上表の小文字を厳守 (Req 5.4)。スキーマは `aws_s3tables_table` の `metadata { iceberg { schema { field … } } }` ブロックで宣言する (AWS provider v6.4+)。これにより作成時に Iceberg メタデータ (`metadata_location`) が初期化され、Firehose の Iceberg 配信がコミット先を解決できる。スキーマ未宣言だと `metadata_location` が空のまま作られ、配信が成立せず Athena も `missing [metadata_location]` で失敗する。
- **Glue Iceberg** (`Glue_Iceberg`): Glue database + table を作成し、`table_type=ICEBERG`、`location` は S3 実体バケットの prefix を指定 (Req 6.1, 6.2)。スキーマは上表と同一。`open_table_format_input.iceberg_input.metadata_operation=CREATE` で作成時に初期メタデータを生成する。Firehose 書き込み後は `metadata_location` 等が書き込みエンジン側で更新されるため、Terraform 側は `lifecycle.ignore_changes = [parameters, storage_descriptor]` でドリフトを無視する。
...
- **AWS provider 制約**: `~> 6` であること (S3 Tables テーブルの `metadata` スキーマ定義は v6.4 で追加されたため必須)。
- **ECS 常駐構成**: service `launchType=FARGATE`、`desiredCount>=1` (Req 2.1, 2.4)。
- **FireLens 構成 (Terraform)**: タスク定義に `firelensConfiguration` サイドカーが存在し、app コンテナの `logDriver=awsfirelens` (Req 3.1, 3.2)、Fluent Bit 設定に `kinesis_firehose` OUTPUT が存在 (Req 3.4)。
- **FireLens ルーティング (fluent-bit/ 静的ガード)**: `custom.conf` が (a) `log` を parser 展開 (`Key_Name log`)、(b) `$severityText ^(ERROR|FATAL)` で振り分け、(c) **整数 `$severityNumber` を rewrite_tag の Rule に使っていない** (整数は正規表現にマッチせず無音故障する既知の罠の回帰ガード)、(d) Iceberg 整形 Lua を呼び出す、こと。`iceberg_transform.lua` が小文字フラットスキーマ (`severity_number` / `resource_json` 等) へ整形すること。
- **S3 Tables スキーマ**: `aws_s3tables_table` が Iceberg スキーマ (`schema.field` 7 列) を宣言していること (`metadata_location` 初期化に必須; Req 5.4/5.5)。
...
> 上記の静的アサーションは `infra/verify.sh` に実装されている (PASS=35 / FAIL=0)。
