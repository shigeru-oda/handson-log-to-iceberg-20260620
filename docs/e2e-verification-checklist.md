...
本書は、Terraform でインフラをデプロイし、ECS Fargate 上の Log_Generator を一定時間
...
1. **S3 (full-logs バケット)** に **全 severity (TRACE〜FATAL)** のログが蓄積される (Req 4.2, 4.3)。
2. **CloudWatch Logs (errors ロググループ)** には **ERROR / FATAL のみ** が現れる (Req 4.1)。
3. **S3 Tables Iceberg テーブル** と **Glue Iceberg テーブル** を Athena でクエリすると、
...
# S3 / CloudWatch / Firehose / Iceberg のリソース名 (terraform output 由来)
...
# S3 Tables Iceberg (マネージド) のカタログ識別子
...
| 種別 | 名前 (例) |
| --- | --- |
| S3 full-logs バケット | `otel-log-pipeline-dev-full-logs-<account_id>-ap-northeast-1` |
| S3 Glue Iceberg データ実体バケット | `otel-log-pipeline-dev-glue-iceberg-<account_id>-ap-northeast-1` |
| CloudWatch Logs ロググループ | `/ecs/otel-log-pipeline-dev/errors` |
| Firehose full-logs ストリーム | `otel-log-pipeline-dev-full-logs` |
| Firehose s3tables-iceberg ストリーム | `otel-log-pipeline-dev-s3tables-iceberg` |
| Firehose glue-iceberg ストリーム | `otel-log-pipeline-dev-glue-iceberg` |
| S3 Tables バケット / namespace / table | `otel-log-pipeline-dev-s3tables` / `otel_log_pipeline_dev` / `error_logs` |
| Glue database / table | `otel_log_pipeline_dev_logs` / `errors` |
| ECS クラスタ / サービス | `otel-log-pipeline-dev-cluster` / `otel-log-pipeline-dev-service` |
...
- [ ] **1-2-2. ECR リポジトリ作成・ビルド・プッシュ (fluent-bit)**
  ```bash
...
- [ ] **1-3-1. init / validate / plan / apply**
  ...
  terraform apply \
    ...
    -var="log_interval_ms=1000"
  ```
  期待結果: `apply` が成功し、VPC/サブネット/SG、ECS クラスタ・サービス・タスク定義、
  Firehose 3 ストリーム、S3 バケット 2 種、CloudWatch ロググループ、S3 Tables テーブル、
  Glue database/table、IAM ロール群が作成される。
  > 注意: ローカルステートにはロック機構がない (Req 7.4)。複数人で同時に `apply` しない
  > こと。

- [ ] **1-3-2. ECS サービスが稼働中であることを確認**
  ```bash
...
- [ ] **1-3-3. 蓄積待ち**
  Firehose のバッファリング (full-logs は最大 300 秒 / Iceberg は 60〜300 秒) を考慮し、
  **最低 10〜15 分** 程度アプリを稼働させてから以降の検証を行う。

---

## 2. 検証 (1): S3 full-logs バケットに全 severity が蓄積される

関連要件: **4.2 (全ログを S3 へ)**, **4.3 (severity による絞り込みなし)**

Firehose `full-logs` ストリームは `raw/!{timestamp:yyyy/MM/dd/HH}/` プレフィックス
...
## 3. 検証 (2): CloudWatch Logs (errors) に ERROR/FATAL のみが現れる
...
- [ ] **3-3. 非エラーログが存在しないことを確認 (ネガティブ検証)**
  ...
  aws logs filter-log-events \
    ...
    --query 'length(events)'
  ```
  期待結果: `0` (非エラーレベルのレコードは CloudWatch Logs に存在しない)。
  > 補足: `firelens` プレフィックスのストリームには log_router 自身の診断ログが入る
  > 場合がある。アプリログの検証では `ecs-` プレフィックスのストリームを対象とすること。

---

## 4. 検証 (3): Iceberg テーブル (S3 Tables / Glue) にエラーログのみが存在する

関連要件: **5.2 (Error_Log を S3 Tables Iceberg へ)**, **6.3 (Error_Log を Glue Iceberg へ)**

Athena (エンジン v3) でクエリする。事前に Athena のクエリ結果出力先 S3 を設定しておく
こと (ワークグループのデフォルト or 下記の `--result-configuration`)。

```bash
# Athena クエリ結果の出力先 (full-logs バケットを流用)
...
### 4-A. Glue Iceberg テーブル (セルフマネージド)
...
### 4-B. S3 Tables Iceberg テーブル (マネージド)

S3 Tables は Athena から S3 Tables カタログ経由でクエリする。カタログは Lake Formation /
S3 Tables 統合により参照する。Athena では S3 Tables 用カタログ
(例: `s3tablescatalog/otel-log-pipeline-dev-s3tables`) を指定し、namespace を database、
table を `error_logs` として扱う。

> 事前準備: S3 Tables を Athena/Glue から参照するための統合
> (`aws s3tables` の Glue federated catalog 連携 / Lake Formation 権限付与) が有効である
> こと。コンソールの Athena クエリエディタでデータソースとして S3 Tables カタログを
> 選択するのが最も確実。

- [ ] **4-B-1. 行数とエラーのみ条件を確認 (Athena コンソール推奨)**
  Athena クエリエディタで、データソースに S3 Tables カタログ、データベースに
  `otel_log_pipeline_dev` を選択し以下を実行:
  ```sql
...
- [ ] **4-B-3. Glue 側との整合 (両ターゲット等価性) を確認**
  期待結果: S3 Tables Iceberg と Glue Iceberg の両テーブルで、エラーログのみが行として
...
- [ ] **5-1.** S3 full-logs に全 severity が蓄積されている (検証 1 / Req 4.2, 4.3)。
- [ ] **5-2.** CloudWatch Logs (errors) に ERROR/FATAL のみが現れる (検証 2 / Req 4.1)。
- [ ] **5-3.** S3 Tables Iceberg テーブルにエラーログのみが行として存在する (検証 3 / Req 5.2)。
- [ ] **5-4.** Glue Iceberg テーブルにエラーログのみが行として存在する (検証 3 / Req 6.3)。

すべてチェックできれば、ステージ 1 (S3 / CloudWatch) とステージ 2 (S3 Tables / Glue
...
- **S3 / CloudWatch / Iceberg にデータが出ない**: Firehose バッファリング待ち時間が
  経過しているか、ECS タスクが `RUNNING` か、タスクロールに `firehose:PutRecordBatch` /
  CloudWatch Logs 権限があるかを確認する。
- **Iceberg テーブルにデータが出ない / 配信失敗**: Firehose の配信失敗レコードは
  バックアップ S3 (`firehose-s3tables-iceberg-errors/` や glue-iceberg バケットの
  `errors/`) に退避される。レコードのキー名が小文字カラムスキーマ
  (`event_time` 等) と一致しているか (スキーママッピング) を確認する。
- **CloudWatch に非エラーが混入**: Fluent Bit の `rewrite_tag` ルール
  (`$severityNumber ^(1[7-9]|2[0-4])$`) と各 OUTPUT の `Match` を確認する。
- **後片付け**: 検証完了後は `terraform destroy` で課金リソース (Firehose / ECS / S3 Tables
  等) を削除する。S3 バケットにオブジェクトが残っていると destroy が失敗するため、
  必要に応じて中身を空にしてから実行する。
