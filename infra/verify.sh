...
# Fluent Bit 設定 (severity ルーティング / Iceberg 整形) も検証対象に含める。
# これらは Iceberg/CloudWatch への配信成立に直結し、過去に「rewrite_tag が整数
# severityNumber にマッチせず全 ERROR ルーティングが無音で停止」という回帰があったため、
# 静的アサーションでガードする。
FLUENT_BIT_DIR="$(cd "${INFRA_DIR}/../fluent-bit" 2>/dev/null && pwd || true)"
CUSTOM_CONF="${FLUENT_BIT_DIR}/custom.conf"
LUA_TRANSFORM="${FLUENT_BIT_DIR}/iceberg_transform.lua"
...
# assert_file_contains <file> <regex> <description>
#   指定ファイルに regex がマッチすれば PASS (ファイル未存在は FAIL)
assert_file_contains() {
  local file="$1" pattern="$2" desc="$3"
  if [[ -f "$file" ]] && grep -Eq -- "$pattern" "$file"; then
    ok "$desc"
  else
    ng "$desc  (file: ${file##*/}, pattern: $pattern)"
  fi
}

# assert_file_not_contains <file> <regex> <description>
#   指定ファイルに regex がマッチしなければ PASS (回帰パターンの不在を保証)
assert_file_not_contains() {
  local file="$1" pattern="$2" desc="$3"
  if [[ -f "$file" ]] && grep -Eq -- "$pattern" "$file"; then
    ng "$desc  (file: ${file##*/}, 不要なパターン検出: $pattern)"
  else
    ok "$desc"
  fi
}
...
# S3 Tables テーブルが Iceberg スキーマを宣言していること (Req 5.4/5.5)。
#   metadata.iceberg.schema.field を宣言しないと metadata_location が初期化されず、
#   Firehose の Iceberg 配信が成立せず Athena が "missing [metadata_location]" で失敗する。
#   provider v6.4+ の metadata ブロックが必須 (下の provider 制約アサーションも参照)。
assert_count_ge 'field[[:space:]]*\{'                             7 'S3 Tables テーブルが Iceberg スキーマ (schema.field) を 7 列宣言している (metadata_location 初期化に必須)'
...
# AWS provider 制約 ~> 6 (aws_s3tables_table の metadata スキーマ定義は v6.4 で追加)
assert_contains 'version[[:space:]]*=[[:space:]]*"~> 6'           'AWS provider 制約が ~> 6 (S3 Tables スキーマ定義に必須)'
...
echo
echo "-- Fluent Bit ルーティング / Iceberg 整形 (fluent-bit/) --"
# FireLens は app の JSON を record の "log" キーに文字列で渡すため、parser で
# トップレベルへ展開する必要がある (Req 3.2/3.3 の前提)。
assert_file_contains "$CUSTOM_CONF" 'Key_Name[[:space:]]+log' \
  'custom.conf が "log" フィールドを parser で展開している (FireLens の JSON 文字列対策)'
# severity 判定は文字列の severityText で行う。rewrite_tag の正規表現は整数フィールド
# ($severityNumber) にマッチしないため、ERROR/FATAL を severityText で振り分ける。
assert_file_contains "$CUSTOM_CONF" '\$severityText[[:space:]]+\^\(ERROR\|FATAL\)' \
  'custom.conf が severityText (^(ERROR|FATAL)) で ERROR/FATAL を振り分けている'
# 回帰ガード: rewrite_tag の Rule で整数 $severityNumber を使っていないこと
# (整数はマッチせず全 ERROR ルーティングが無音で停止する既知の罠)。
assert_file_not_contains "$CUSTOM_CONF" '^[[:space:]]*Rule[[:space:]]+\$severityNumber' \
  'custom.conf の rewrite_tag が整数 $severityNumber を使っていない (既知の罠の回帰ガード)'
# Iceberg 向けに Lua でスキーマ整形して配信する。
assert_file_contains "$CUSTOM_CONF" 'iceberg_transform\.lua' \
  'custom.conf が Iceberg 整形 Lua (iceberg_transform.lua) を呼び出している'
# Lua が小文字フラットスキーマ (Iceberg カラム) へ整形している。
assert_file_contains "$LUA_TRANSFORM" 'severity_number' \
  'iceberg_transform.lua が小文字フラットスキーマ (severity_number 等) へ整形している'
assert_file_contains "$LUA_TRANSFORM" 'resource_json' \
  'iceberg_transform.lua が resource を JSON 文字列カラム (resource_json) へ整形している'
